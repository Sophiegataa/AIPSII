// let média, nota1, nota2

// nota1 = Number(prompt("digit a sua primeira nota"))
// nota2 = Number(prompt("digte sua segunda nota"))
// média = (nota1+nota2)/2

// if(média > 7){
//     alert("Aprovado")
// }else{
//     alert("Reprovado")
// }
// alert("suamédia final é: " + média)



// let potuação

// pontuação =  Number(prompt("qual foi a sua pontuação"))
// if(pontuação <= 10){
//     alert("deu ruim")
// }
// if(pontuação <10 && pontuação <= 100){
//     alert("ta mais ou menos")
// }
// if(pontuação >100 && pontuação <= 200){
//     alert("ta bonzin")
// }
// if(pontuação >200){
//     alert("uiiiiiiiii, muito boooooooooooooommmmmmmm")
// }



// let dias,diarias, multa, desconto

// dias = Number(prompt("Quantos dias vai ficar? "))
// multa = Number(150)
// desconto = Number(0.25)

// if(dias <= 5){
//     diarias = ((100*dias) + multa)*desconto
//     alert("Sua conta ficou: " + diarias)

// }
// if(dias >5 && dias <= 10){
//     diarias = ((90*dias) + multa)*desconto
//     alert("Sua conta ficou: " + diarias)

// }
// if(dias >10){
//     diarias = ((80*dias) + multa)*desconto
//     alert("Sua conta ficou: " + diarias)

// }